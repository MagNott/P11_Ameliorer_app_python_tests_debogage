...
def test_email_not_valid(client):
    response = client.post(
        "/showSummary",
        data={"email": "clubconnnu@example.com"},
...
def test_purchase_club_has_enough_points(
    ...
    response = client.post(
        "/purchasePlaces",
        data={"competition": nom_competition, "club": nom_club, "places": "2"},
        follow_redirects=True,
    )
...
def test_show_clubs_displays_club_list_successfully(client, clubs_data):
